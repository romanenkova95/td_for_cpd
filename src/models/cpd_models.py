...
class CPDModel(pl.LightningModule):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if self.extractor:
            ...
            outputs = self.model(outputs.transpose(1, 2))
        else:
    ...
    def test_epoch_end(self, outputs: List[Any]):
        ...
        self.test_predictions = torch.stack(self.test_predictions).squeeze().flatten(0, 1)
        self.test_labels = torch.stack(self.test_labels).squeeze().flatten(0, 1)

        for t, threshold in enumerate(self.thresholds):
            TN[t], FP[t], FN[t], TP[t], FP_delay, delay, cover = calculate_metrics(
                self.test_labels, self.test_predictions > threshold
            )
            FP_delays[t] = FP_delay.float().mean().detach().cpu()
            delays[t] = delay.float().mean().detach().cpu()
            covers[t] = cover
